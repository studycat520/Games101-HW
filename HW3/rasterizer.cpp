...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos) 
{
    ...
    min_x = std::floor(min_x); max_x = std::ceil(max_x);
    min_y = std::floor(min_y); max_y = std::ceil(max_y);
    ...
    for (int x = min_x; x <= max_x; ++x) {
        for (int y = min_y; y <= max_y; ++y) {
            ...
            if (insideTriangle(x, y, t.v)) {
                ...
                // 3. 计算重心坐标 (Alpha, Beta, Gamma)
                auto[alpha, beta, gamma] = computeBarycentric2D(x + 0.5f, y + 0.5f, t.v);

                // 4. 透视校正插值系数 Z (View Space Depth)
                // v[i].w() 存储的是 view space z 值
                ...
                if (zp < depth_buf[index]) { 
                    ...
                    // 6. 属性插值 (Attributes Interpolation)
                    // 所有属性插值都需要透视校正，即先除以 w，插值后，再乘以 Z
